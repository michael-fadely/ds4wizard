...
namespace hid
{
	...
	bool enum_guid(const std::function<bool(std::wstring&, std::wstring&)>& fn, const GUID& guid) noexcept;
	void enum_hid(const std::function<bool(HidInstance&)>& fn) noexcept;
	void enum_usb(const std::function<bool(std::wstring&, std::wstring&)>& fn) noexcept;
}
